...
const createGroup = async (groupData) => {
  try {
    ...
    const response = await my.request({
      ...
      dataType: 'json',
      success: (res) => {
        console.log('Group creation success response:', JSON.stringify(res));
      },
      fail: (err) => {
        console.error('Group creation fail response:', JSON.stringify(err));
      }
    });
    
    console.log('Group created successfully:', JSON.stringify(response.data));
    return response.data;
  } catch (error) {
